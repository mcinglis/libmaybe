...
$(gen_headers): $(prefix)%.h: header.h.jinja
	$(eval up := $(call uc,$*))
	$(RENDER_JINJA) $< "include_guard=LIBMAYBE_MAYBE_$(up)_H" "sys_headers=libbase/$*.h" "rel_headers=" "type=$(or $($(*)_type),$*)" "macroname=$(up)" "typename=$*" "funcname=$*" "typeclasses=$(common_typeclasses) $($(*)_typeclasses)" -o $@
